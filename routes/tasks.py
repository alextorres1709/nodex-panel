...
TASK_TEMPLATES = {
    "web": {
        "name": "Proyecto Web",
        "tasks": [
            "Briefing con cliente", "Diseño de wireframes", "Maquetación HTML/CSS",
            "Desarrollo frontend", "Desarrollo backend / API", "Base de datos y modelos",
            "Integración de pagos", "Testing y QA", "Despliegue a producción",
    ...
    "landing": {
        "name": "Landing Page",
        "tasks": [
            "Definir propuesta de valor", "Diseño visual", "Desarrollo responsivo",
            "SEO y meta tags", "Formulario de contacto", "Deploy y DNS",
        ],
    },
    "bot": {
        "name": "Bot WhatsApp",
        "tasks": [
            "Definir flujos de conversación", "Setup Twilio/Meta API",
            "Desarrollo de lógica del bot", "Integración con CRM",
            "Testing de flujos", "Deploy en servidor", "Documentación",
        ],
    },
}


@tasks_bp.route("/tareas/from-template", methods=["POST"])
@login_required
def from_template():
    tpl_key = request.form.get("template", "")
    tpl = TASK_TEMPLATES.get(tpl_key)
    if not tpl:
        flash("Plantilla no encontrada", "error")
        return redirect(url_for("tasks.index"))
    pid = request.form.get("project_id", "").strip()
    for i, title in enumerate(tpl["tasks"]):
        task = Task(
            title=f"[{tpl['name']}] {title}",
            status="pendiente",
            priority="media",
            project_id=int(pid) if pid else None,
            kanban_order=i,
        ...
        db.session.add(task)
    db.session.commit()
    log_activity("create", "task", details=f"Plantilla: {tpl['name']} ({len(tpl['tasks'])} tareas)")
    flash(f"Creadas {len(tpl['tasks'])} tareas desde plantilla '{tpl['name']}'", "success")
    return redirect(url_for("tasks.index"))
